...
function validateOptsAgainstSchema(opts, schema) {
    validateObject(opts, schema.properties || {}, schema.required || [], schema.definitions || {});
}
exports.validateOptsAgainstSchema = validateOptsAgainstSchema;
function validateObject(opts, properties, required, definitions) {
    required.forEach((p) => {
        if (opts[p] === undefined) {
...
function validateProperty(propName, value, schema, definitions) {
    ...
    if (isPrimitive) {
        ...
        if (schema.type !== 'object')
            throwInvalidSchema(propName, schema);
        validateObject(value, schema.properties || {}, schema.required || [], definitions);
    }
}
/**
...
function combineOptionsForExecutor(commandLineOpts, config, target, schema, defaultProjectName) {
    ...
    convertSmartDefaultsIntoNamedParams(combined, schema, commandLineOpts['_'] || [], defaultProjectName);
...
function combineOptionsForGenerator(commandLineOpts, collectionName, generatorName, wc, schema, isInteractive, defaultProjectName) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
...
function convertSmartDefaultsIntoNamedParams(opts, schema, argv, defaultProjectName) {
